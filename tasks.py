...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_DB_DUMPS_xxx
namespace = Collection("nautobot_db_dumps")
namespace.configure(
    {
        "nautobot_db_dumps": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-db-dumps",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_db_dumps.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_db_dumps.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_db_dumps.nautobot_ver,
        "PYTHON_VER": context.nautobot_db_dumps.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_db_dumps.project_name}",
        f'--project-directory "{context.nautobot_db_dumps.compose_dir}"',
    ]

    for compose_file in context.nautobot_db_dumps.compose_files:
        compose_file_path = os.path.join(context.nautobot_db_dumps.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_db_dumps.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_db_dumps.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_db_dumps.python_ver
    dockerfile_path = os.path.join(context.nautobot_db_dumps.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_db_dumps.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_db_dumps.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_db_dumps.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_db_dumps"
...
def backup_db(context, db_name="", output_file="dump.sql", readable=True):
    ...
    if _is_compose_included(context, "mysql"):
        command += [
            ...
            f"--result-file=/tmp/db_dumps/nautobot{context.nautobot_db_dumps.nautobot_ver}-mysqldump.sql",
        ...
        command += [
            "pg_dump",
            "--username=$POSTGRES_USER",
            f"--file=/tmp/db_dumps/nautobot{context.nautobot_db_dumps.nautobot_ver}-pgdump.sql",
            f"--dbname={db_name or '$POSTGRES_DB'}",
        ]
    else:
        raise ValueError("Unsupported database backend.")
...
@task(
    help={
        "version": "Version of Nautobot Database Dumps to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_db_dumps"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_db_dumps") / Path("migrations")
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_db_dumps/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_db_dumps.local):
